...
import Lib.Core.UserSession (SessionToken (..), UserSession (UserSession))
import Lib.Db.Functions (WithDb, asSingleRow, execute, query)

sessionTTL :: NominalDiffTime
sessionTTL = 60 * 60 * 24 * 7 -- 7 days

createSession :: (WithDb env m, WithError m, MonadIO m) => PlayerID -> m UserSession
createSession pID = do
    ...
    pure $ UserSession token pID expiresAt
